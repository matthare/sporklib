...
_, incoming_FPID_list , db_dict = sporklib.get_fpid_list(BASE_DIR,ignore_dirs)
_, epub_FPID_list , epub_dict = sporklib.get_fpid_list(EPUB_DIR,ignore_dirs)
_, pdf_FPID_list , pdf_dict = sporklib.get_fpid_list(PDF_DIR,ignore_dirs)
...
for fpid in in_all_set:
    ...
    elif os.path.exists(os.path.join(EPUB_DIR,'error/accept',file_name)):
        file2 = os.path.join(EPUB_DIR,'error/accept',file_name)
    elif os.path.exists(os.path.join(EPUB_DIR,'error/reject',file_name)):
        file2 = os.path.join(EPUB_DIR,'error/reject',file_name)
...
for fpid in in_doc_and_epub_only_set:
    ...
    if file1 != "ERROR3a" and file2 != "ERROR3b":
        if filecmp.cmp(file1,file2):
            file3 = os.path.join('/home/content/uploaded/duplicates',file_name)
